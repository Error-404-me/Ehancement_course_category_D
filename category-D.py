...
def insert_structured_records(cursor, records):
    ...
    rec = [(i["student_id"], i["name"], i["course"], i["quiz"], i["exam"], i["attendance"]) for i in records]
    cursor.executemany("INSERT INTO raw_student_scores (student_id, name, course, quiz, exam, attendance) VALUES (?,?,?,?,?,?)", rec)
...
def insert_unstructured_records(cursor, records):
    ...
    # Write INSERT INTO code here
    for i in records:
        cursor.execute("INSERT INTO raw_student_comments (raw_text) VALUES (?)", (i,))
...
def select_final_report(cursor):
    ...
    cursor.execute("SELECT * FROM final_student_report")
